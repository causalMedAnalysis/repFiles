...
sink(paste(logdir, "figure_3-7_log.txt", sep=""))
...
ggplot(ipw.boot, aes(x=V3)) +
	geom_histogram(binwidth=0.00125, color="black", fill="grey90") +
	#geom_density(adjust=0.8, alpha=0.1, fill="black") +
	geom_vline(xintercept=quantile(ipw.boot[,3], prob=0.025), linetype=2, linewidth=0.5) +
	geom_vline(xintercept=quantile(ipw.boot[,3], prob=0.975), linetype=2, linewidth=0.5) +
 	theme_bw() +
	scale_y_continuous(name="Frequency", limits=c(0, 160), breaks=seq(0, 160, 20)) +
	scale_x_continuous(name=expression(hat(theta)[b]), limits=c(-0.05, 0.02), breaks=round(seq(-0.05, 0.02, 0.01), 2)) 


ggsave(paste(figdir, "figure_3-7.png", sep=""), height=4.5, width=4.5, units="in", dpi=600)
